...
// Send records the pull. Live inventory.batchChange (IN_STOCK → SOLD at
// L6D106R4VNA72) stays off until MAURICE_PICK_SEND_DRY_RUN=0 and
// MAURICE_PICK_LIVE_DEDUCT=1. Until then Send logs the adjustment it would
// have made. Send never creates a Square invoice. Priced lines are appended
// to the week log for the Monday unpaid rollup. Never Payments, never
// Twilio, never the INV001–INV056 counting items.
...
function parseCreateBody(body) {
  ...
  if (body.createInvoice != null && typeof body.createInvoice !== 'boolean') {
    return fail(400, 'createInvoice must be true or false')
  }
...
// Dry-run is the safe default. Unset, empty, or any value other than an
// explicit off (0/false/no/off) keeps Send from calling batchChange.
// Live deduct also requires MAURICE_PICK_LIVE_DEDUCT=1. Either gate left
// closed means dry-run. DRY_RUN=1 wins even if live deduct is set.
function dryRunEnabled(env = process.env) {
  const raw = env && env.MAURICE_PICK_SEND_DRY_RUN
...
function liveInventoryDeductEnabled(env = process.env) {
  if (dryRunEnabled(env)) return false
  return truthyFlag(env && env.MAURICE_PICK_LIVE_DEDUCT)
}
...
function sentResult(record, alreadySent) {
  ...
  return {
    ...
    dryRun: notify.dryRun === true,
    inventoryAdjusted: notify.inventoryAdjusted === true,
    weekLogged: Array.isArray(record.pricedLines) && record.pricedLines.length > 0,
...
function buildPricedLines(sentLines, facts) {
  return sentLines.map(line => {
    const sent = Number(line.qtySent)
    const fact = facts.get(line.sellableCatalogObjectId)
    const unitPriceCents = fact && Number.isInteger(fact.priceCents) ? fact.priceCents : null
    let lineTotalCents = null
    if (sent <= 0) lineTotalCents = 0
    else if (unitPriceCents != null) lineTotalCents = Math.round(sent * unitPriceCents)
    return {
      sellableCatalogObjectId: line.sellableCatalogObjectId,
      name: line.name,
      qtyOrdered: line.qtyOrdered,
...
function buildNotifySummary({ record, sentLines, facts, dryRun }) {
  const shorts = []
  let cents = 0
  let priced = true
...
function toPageProps(record, env = process.env) {
  ...
  const storedDry = record.notify && typeof record.notify.dryRun === 'boolean' ? record.notify.dryRun : null
  const dryRun = sent && storedDry != null ? storedDry : !liveInventoryDeductEnabled(env)
  return {
    missing: false,
    misconfigured: false,
    error: '',
...
async function loadPickPage(token, deps) {
  ...
  return toPageProps(record, (deps && deps.env) || process.env)
}
...
async function sendMauricePick({
  ...
  if (typeof resolveAccount !== 'function' || typeof squareFetch !== 'function') {
  ...
  const adjustment = buildInventoryDecreaseBody({
    idempotencyKey,
    locationId,
    lines: positive,
    occurredAt
  })
  const dryRun = !liveInventoryDeductEnabled(env)
  if (dryRun) {
    console.log(JSON.stringify({
      event: 'maurice_pick_inventory_dry_run',
      dryRun: true,
      locationId,
      adjustment
    }))
    ...
    console.log(JSON.stringify({
      event: 'maurice_pick_inventory_live',
      dryRun: false,
      locationId,
      adjustment
    }))
  ...
  const pricedLines = buildPricedLines(sentLines, facts)
  console.log(JSON.stringify({
    event: 'maurice_pick_week_log',
    token: checked,
    printDay: record.printDay || null,
    date: record.pickDate || null,
    dryRun,
  ...
  const summary = await notifyJordan({
    env,
    fetchImpl,
    summary: buildNotifySummary({ record, sentLines, facts, dryRun })
  })
