...
Every `/api/square/*` route requires `BRIDGE_API_KEY`, except `POST /api/square/wholesale-pull/webhook`. That webhook is signed with `SQUARE_WEBHOOK_SIGNATURE_KEY` (Square cannot send the bridge key). It creates Todoist **Takeout** pull tasks and pick-list PDFs. It does not charge cards. Checking off the Takeout task is a separate route, `POST /api/wholesale-pull/todoist-webhook`, signed with `TODOIST_WEBHOOK_SECRET`. That route queues a signature invoice PDF. It does not call Square Payments. See [wholesale-pull.md](wholesale-pull.md). `PICK_MINT_API_KEY` is not accepted on bridge routes. That secret only authorizes `POST /api/pick/maurice-restock/create` (see [maurice-restock-pick-qr.md](maurice-restock-pick-qr.md)).
