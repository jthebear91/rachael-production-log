...
async function renderPickSheet({ pick, pickUrl }) {
  ...
  const rows = (pick.lines || []).map(line => {
    const name = escapeHtml(line.name || line.catalogObjectId)
    const ordered = escapeHtml(line.orderedQty)
    return `<tr><td class="name">${name}</td><td class="qty">${ordered}</td></tr>`
  }).join('')
  const note = pick.note ? `<p class="note">${escapeHtml(pick.note)}</p>` : ''
  const bannerHtml = banner ? `<p class="banner">${escapeHtml(banner)}</p>` : ''
  const when = pick.createdAt ? escapeHtml(chicagoStamp(pick.createdAt)) : ''
  return `<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8">
...
<body>
  <main>
    ...
    <p class="sub">Wholesale pick list${when ? ` · ${when}` : ''}</p>
    ${bannerHtml}
    ${note}
    ...
    <p>After the pull, scan the code. Lower any short lines on the phone, then Send.</p>
    <table>
