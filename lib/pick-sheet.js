...
const { ALREADY_SENT_LABEL, appBaseUrl, getMauricePick, liveInventoryDeductEnabled } = require('./maurice-pick')
...
async function renderPickSheet({ pick, pickUrl, dryRun = true }) {
  ...
  const inventoryNote = dryRun
    ? 'Dry run is on. Send records the pull and logs the adjustment. It does not change wholesale inventory.'
    : 'Inventory is not changed until Send.'
  const estimate = pick.estimatedTotal
    ? `<p class="note">Estimated total $${escapeHtml(pick.estimatedTotal)}. ${inventoryNote}</p>`
    : `<p class="note">${inventoryNote}</p>`
  const sendNote = dryRun
    ? 'After the pull, scan the code. Leave full lines alone. Lower only the shorts, then Send. Dry run is on, so Send does not deduct inventory.'
    : 'After the pull, scan the code. Leave full lines alone. Lower only the shorts, then Send. Send deducts the final quantities at the wholesale location.'
...
async function loadPickSheet({ token, req, env = process.env, store } = {}) {
  ...
  const html = await renderPickSheet({ pick, pickUrl, dryRun: !liveInventoryDeductEnabled(env) })
  return html
}
