// Shared Square HTTP client for the production-log app and the read-only
// /api/square/* bridge. Account selection:
//   wholesale  (default) → SQUARE_TOKEN + SQUARE_LOCATION_ID
//   restaurant           → SQUARE_RESTAURANT_TOKEN + SQUARE_RESTAURANT_LOCATION_ID
//
...
export function resolveAccount(accountParam) {
  const account = String(accountParam || 'wholesale').toLowerCase()
  if (account !== 'wholesale' && account !== 'restaurant') {
    throw httpError(400, 'account must be wholesale or restaurant')
  }

  if (account === 'restaurant') {
    const token = process.env.SQUARE_RESTAURANT_TOKEN
    const locationId = process.env.SQUARE_RESTAURANT_LOCATION_ID
    if (!token) {
      throw httpError(503, 'Restaurant Square account is not configured')
    }
    return { account, token, locationId: locationId || null }
  }
