...
function buildTaskTitle({ account, reference }) {
  const title = `PULL · ${account || 'Wholesale account'} · ${reference || 'order'}`
...
async function finishPull({ invoice, order, payment, customer, squareFetch, token }) {
  ...
  const title = buildTaskTitle({ account, reference })
...
async function resolveInvoice({ invoice, env, account, squareFetch, force }) {
  ...
  return finishPull({ invoice, order, payment: null, customer, squareFetch, token: account.token })
}
...
async function resolvePayment({ payment, env, account, squareFetch, force }) {
  ...
  return finishPull({ invoice: null, order, payment, customer, squareFetch, token: account.token })
}
...
async function resolveOrder({ order, env, account, squareFetch, force }) {
  ...
  return finishPull({ invoice: null, order, payment: null, customer, squareFetch, token: account.token })
}
