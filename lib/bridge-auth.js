...
import { isSalesAuthenticated, salesUnauthorizedMessage } from './sales-auth.js'
import { sanitizeErrorMessage } from './square-client.js'
...
export function authorizePickMint(req, res) {
  // POST /api/pick/maurice-restock/create only. BRIDGE_API_KEY or PICK_MINT_API_KEY.
  // Inventory, Send, invoices, and other bridge routes stay on authorizeBridge.
  const bridgeKey = process.env.BRIDGE_API_KEY
  const mintKey = process.env.PICK_MINT_API_KEY
  if (!bridgeKey && !mintKey) {
    res.status(503).json({ error: 'Bridge is not configured' })
    return false
  }
  const provided = providedBridgeKey(req)
  const bridgeOk = Boolean(bridgeKey) && safeEqual(provided, bridgeKey)
  const mintOk = Boolean(mintKey) && safeEqual(provided, mintKey)
  if (!bridgeOk && !mintOk) {
    res.status(401).json({ error: 'Unauthorized' })
    return false
  }
